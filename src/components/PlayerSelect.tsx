...
export default function PlayerSelect() {
  const t = useTranslations('playerSelect');
  const { setSelectedPlayer, setGameMode } = useQuiz();

  return (
    <div className="w-full max-w-2xl mx-auto">
      <div className="mb-6">
        <h2 className="text-xl sm:text-2xl font-bold text-dark-300 mb-2">{t('whichPlayer')}</h2>
        <p className="text-dark-200 text-sm md:text-base">
          {t('scoresSavedSeparately')}
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-dark-300">{t('player1')}</div>
            <div className="text-sm text-dark-200">{t('imPlayer1')}</div>
          </div>
        </button>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-dark-300">{t('player2')}</div>
            <div className="text-sm text-dark-200">{t('imPlayer2')}</div>
          </div>
        </button>
      </div>
